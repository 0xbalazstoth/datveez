...
import { removeNumbers } from "@/app/utils/utils";
...
function NumbersNode() {
  ...
  const defaultText = "Test123";
  const normalizedText = removeNumbers(defaultText);

  const [oldText, setOldText] = useState<string>(defaultText);
  const [newText, setNewText] = useState<string>(removeNumbers(oldText));

  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    setOldText(value);
    setNewText(removeNumbers(value));
  };
    ...
    <>
      ...
      >
        <Handle
          ...
          id={typeOfNode.NumbersNode.nodeHandlerId}
        />
        <Handle
          type="source"
...
export default memo(NumbersNode);
